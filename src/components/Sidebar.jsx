import React, {} from "react";
import { NavLink } from "react-router-dom";
// eslint-disable-next-line arrow-parens
function Sidebar(){
      return (
            <div>
                 {/* Main Sidebar Container */}
                  <aside className="main-sidebar sidebar-dark-primary elevation-4">
                  {/* Brand Logo */}
                  <NavLink to="/" className="brand-link">
                  <img src="/dist/img/AdminLTELogo.png" alt="AdminLTE Logo" className="brand-image img-circle elevation-3" style={{opacity: '.8'}} />
                  <span className="brand-text font-weight-light">AdminLTE 3</span>
                  </NavLink>
                  {/* Sidebar */}
                  <div className="sidebar">
                  {/* Sidebar user panel (optional) */}
                  <div className="user-panel mt-3 pb-3 mb-3 d-flex">
                        <div className="image">
                        <img src="/dist/img/user2-160x160.jpg" className="img-circle elevation-2"  />
                        </div>
                        <div className="info">
                        <NavLink to="/" className="d-block">Alexander Pierce</NavLink>
                        </div>
                  </div>
                  {/* SidebarSearch Form */}
                  <div className="form-inline">
                        <div className="input-group" data-widget="sidebar-search">
                        <input className="form-control form-control-sidebar" type="search" placeholder="Search" aria-label="Search" />
                        <div className="input-group-append">
                        <button className="btn btn-sidebar">
                              <i className="fas fa-search fa-fw" />
                        </button>
                        </div>
                        </div>
                  </div>
                  {/* Sidebar Menu */}
                  <nav className="mt-2">
                        <ul className="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        {/* Add icons to the links using the .nav-icon class
                        with font-awesome or any other icon font library */}
                        <li className="nav-item menu-open">
                        <NavLink to="/" className="nav-link active">
                              <i className="nav-icon fas fa-tachometer-alt" />
                              <p>
                              Dashboard
                              
                              </p>
                        </NavLink>
                        </li>
                        <li className="nav-item">
                        <NavLink to="/productLits" className="nav-link">
                              <i className="nav-icon ion ion-bag " />
                              <p>
                              Products
                              <span className="right badge badge-success">20</span>
                              </p>
                        </NavLink>
                        </li>

                        <li className="nav-item">
                        <NavLink to="/newProduct" className="nav-link">
                              <i className="nav-icon ion ion-pie-graph" />
                              <p>
                              Add Product
                              <span className="right badge badge-danger">New</span>
                              </p>
                        </NavLink>
                        </li>

                        <li className="nav-item">
                        <NavLink to="/categories" className="nav-link">
                              <i className="nav-icon fas fa-th" />
                              <p>
                              Categories
                              <span className="right badge badge-success">25</span>
                              </p>
                        </NavLink>
                        </li>

                        <li className="nav-item">
                        <NavLink to="/orders" className="nav-link">
                              <i className="nav-icon fas fa-cart-plus" />
                              <p>
                              Orders
                              <span className="right badge badge-success">100</span>
                              </p>
                        </NavLink>
                        </li>

                        <li className="nav-item">
                        <NavLink to="/users" className="nav-link">
                              <i className="nav-icon fas fa-user" />
                              <p>
                              Users
                              <span className="right badge badge-success">30</span>
                              </p>
                        </NavLink>
                        </li>

                        <li className="nav-item">
                        <NavLink to="/sellers" className="nav-link">
                              <i className="nav-icon fas fa-users" />
                              <p>
                              Sellers
                              <span className="right badge badge-success">20</span>
                              </p>
                        </NavLink>
                        </li>

                        <li className="nav-item">
                        <NavLink to="/transactions" className="nav-link">
                              <i className="nav-icon fas fa-money-bill" />
                              <p>
                              Transactions
                              <span className="right badge badge-success">10</span>
                              </p>
                        </NavLink>
                        </li>
                       
                        </ul>
                  </nav>
                  {/* /.sidebar-menu */}
                  </div>
                  {/* /.sidebar */}
                  </aside>

            </div>     
      )
}

export default Sidebar;